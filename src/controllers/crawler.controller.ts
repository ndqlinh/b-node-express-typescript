...
export const crawlMultipleUrls = async (
  ...
  try {
    ...
    if (!requestBody.urlListSelector) {
      return BaseResponse.toError(
        new HttpException(HTTPStatus.BAD_REQUEST, 'urlListSelector is required')
      );
    }

    if (!requestBody.contentSelector) {
      return BaseResponse.toError(
        new HttpException(HTTPStatus.BAD_REQUEST, 'contentSelector is required')
      );
    }
    ...
    return BaseResponse.toError(
      new HttpException(
        ...
        'Error processing crawl request'
      )
    );
  }
};
